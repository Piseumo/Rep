...
public class JwtUtils {
    ...
    public JwtUtils(Environment environment) {
        ...
        this.accessTokenExpirationTime = Integer.parseInt(this.environment.getProperty("token.access_token_expiration_time"));
        this.refreshTokenExpirationTime = Integer.parseInt(this.environment.getProperty("token.refresh_token_expiration_time"));
    ...
    public String createRefreshToken(String email) {
        String jwt = Jwts.builder()
                .claim("email", email)
                .issuedAt(new Date(System.currentTimeMillis()))
                .expiration(new Date(System.currentTimeMillis() + this.refreshTokenExpirationTime)) // 1초*60*60*24 1일 유효함
                .signWith(secretKey)
                .compact();
        return jwt;
    }
